...
function stopBot() {
  botRunning = false;
  broadcast("⛔ Robô parado manualmente.");
}

wss.on("connection", socket => {
  socket.on("message", msg => {
    if (msg === "toggle-mode") switchMode();
    if (msg === "stop-bot") stopBot();
  });
});
...
async function analyzeSymbol(symbol) {
  try {
    ...
    const { price, ema, rsi, macd, signal, boll, volume, averageVolume } = indicators;

    broadcast({ symbol, price });

    const log = `[${symbol}] 💹 Price: ${price.toFixed(4)} | EMA: ${ema.toFixed(4)} | RSI: ${rsi.toFixed(2)} | MACD: ${macd.toFixed(4)} | Signal: ${signal.toFixed(4)} | Band: ${boll.band} | Vol: ${volume}`;
    broadcast(log);
    ...
    const stopLossTriggered =
      state[symbol].opened &&
      price < state[symbol].entryPrice * (1 - STOP_LOSS_PERCENT);
...
async function startBot() {
  ...
  broadcast(`📊 Análise: ${new Date().toLocaleTimeString()} | Modo: ${MODE.toUpperCase()}`);
  await getBalance();
